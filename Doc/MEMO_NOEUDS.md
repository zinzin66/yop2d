...
- Liste complète des formules (opérateurs, `!=`, `random(1,6)`, `implique.x`, fonctions) : voir `REFERENCE_NOEUDS.md`.
- Dans un script de PREFAB : ne jamais écrire le nom de l'objet dans une formule (`ennemi.x`, `ennemi.y`) — chaque copie reçoit un nom unique (`ennemi_9fc18`...), la formule répond « Objet introuvable ». En revanche la cible d'un nœud (`cibleNom: "ennemi"`) fonctionne, et après une collision ou un clic on peut lire l'objet concerné avec `implique.x`, `implique.y`… Préférer `pousser` (ajout relatif) à `glisser_vers`/`deplacer` (position absolue, qui demandent de connaître la position actuelle).

## 6. Méthode de test
- Script JSON `{"noeuds":[...],"liens":[...]}` collé avec le bouton `{ }`, deux choix :
  - **Ajouter** : les nœuds collés s'ajoutent au script existant, placés en dessous ; un identifiant déjà pris est renommé automatiquement (`n1` → `n1_2`), fils compris. Marche aussi sur un script vide. On peut donc ne donner que les nœuds nouveaux.
  - **Remplacer** : le script entier est remplacé.
  - Le journal confirme : `IMPORT_AJOUT : X noeud(s) et Y lien(s) ajoutes`.
...
- Nœud : `{"id":"a1","classeType":"com.ludexa.moteur.NoeudGenerique","cleNoeud":"<cle>","x":0,"y":0,"parametres":{"<cle>.<champ>":"valeur"},"cibleNom":"player"}`.
- **Cible par tag** : `"cibleNom": "tag:ennemi"` → le nœud agit sur TOUS les objets portant ce tag (clones de prefab compris), puis passe une seule fois au nœud suivant. Marche pour tout nœud du catalogue avec une cible Objet A (pas encore pour Objet B). Dans l'éditeur : liste « Objet A », en bas, les tags affichés `# ennemi`.
- **Toujours donner `x`/`y` espacés** à chaque nœud (environ 350 entre deux étapes, 400 entre deux événements) : sans position, tous les nœuds s'empilent au même endroit et les fils peuvent se perdre à l'import.
- **Jamais deux fils depuis la même sortie** : un seul des deux chemins est suivi. Pour lancer plusieurs actions depuis un même événement, passer par un nœud `sequence` (sorties `port_1`, `port_2`... dans l'ordre).
...
- Collision : les lignes `COLLISION_TAG_CHECK` du journal sont de simples vérifications ; le vrai déclenchement est `COLLISION_TAG DECLENCHEMENT`, suivi de `FIN_CONTACT` quand le contact cesse.
- Zone de bord trop fine : un objet rapide peut la traverser sans la toucher. Faire des murs larges qui dépassent de l'écran.
- Variable locale à un prefab = une variable PAR copie. Pour qu'un groupe bouge ensemble (rangée d'ennemis), utiliser une variable GLOBALE (ex `sens`).
- Pour une action sur tout un groupe (ex toute la rangée descend quand un ennemi touche le mur) : mettre la collision dans la scène principale (Cible = le mur, Tag = ennemi) et agir avec la cible `tag:ennemi`.
- Rotation de l'écran de la tablette sans sauvegarde = tout est perdu (bug connu, pas encore corrigé). Sauvegarder souvent.

## 8. Fichiers à fournir en début de conversation

**Systématiquement** (base commune à presque tout) :
- Ce mémo
- `REFERENCE_NOEUDS.md` (clés, champs, cibles et sorties des 97 nœuds, événements, formules)
...
- Cible par tag (`tag:xxx`) ajoutée et testée : toute une rangée d'ennemis (prefab) descend d'un coup. Code : `NoeudBase` (outils tag), `NoeudGenerique.executer()` (boucle), `EditeurNoeudDialog` (tags dans la liste Objet A).
- Nœud « Tirer » : direction en angles `0`/`90`/`180`/`270` (fait, testé). Ancien fichier `NoeudActionTirer.java` = code mort, ne plus le modifier ; le vrai code est `ActionsObjets.tirer()`.
